...
def test_cache_paths_resolve_from_config_root(tmp_path: Path) -> None:
    ...
    assert market_cache_file(cfg, "1m").name == "market_USDTBRL_1m.json"
    assert runtime_status_cache_file(cfg).name == "runtime_status_USDTBRL.json"
    assert open_orders_cache_file(cfg).name == "open_orders_USDTBRL.json"
